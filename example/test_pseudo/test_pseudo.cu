...
int main(){
    ...
    BIGNUM *open_a, *open_b, *open_c, *open_result, *pow_2_w, *open_mid_value1,*open_mid_value2;
    BN_WORD *bn_a, *bn_b, *bn_c, *bn_result, *bn_word_result,*mad_lo_u,*mad_lo_v,*mid_value1,*mid_value2,
	    *mid_value3,*mad_hi_u,*mad_hi_v,*temp_result,*c_2dmax,*a_half,*b_half;
    BN_CTX *ctx;
    int transform_result;
    int *add_return_value, *mul_lo_return_value, *mad_lo_return_value, *shift_return_value;
// test mul_lo
    ...
    cudaMallocManaged((void**)&(mul_lo_return_value),sizeof(int));
    mul_lo(bn_a,bn_b,bn_word_result,mul_lo_return_value);
    if(*(mul_lo_return_value)!=0){
...
// test mad_lo
    ...
    BN_mod_mul(open_result,open_a,open_b,pow_2_w,ctx);
    BN_add(open_result,open_result,open_c);
    bn_a=BN_WORD_new(DMAX);
    bn_b=BN_WORD_new(DMAX);
    ...
    cudaMallocManaged((void**)&(mad_lo_return_value),sizeof(int));
    cudaMallocManaged((void**)&(mul_lo_return_value),sizeof(int));
    mad_lo(bn_a,bn_b,bn_c,mad_lo_u,mad_lo_v,mad_lo_return_value,mul_lo_return_value,add_return_value);
    if(*(mul_lo_return_value)!=0){
        cerr<<"Error: add failed"<<endl;
        exit(1);
    }
...
//test mad_hi
    ...
    mad_hi(bn_a, bn_b, bn_c, mad_hi_u, mad_hi_v,a_half,b_half,bn_word_result,mid_value1,mid_value2,mid_value3,temp_result,c_2dmax, add_return_value,shift_return_value);
    cout<<"bn_word_result_u"<<endl;
